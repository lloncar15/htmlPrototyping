...
const material = new THREE.MeshStandardMaterial({
  ...
  normalScale: new THREE.Vector2(1, 1),
  aoMap: aoTexture, // Ambient occlusion (uses `uv` by default)
  aoMapIntensity: 1,
...
// Since r151 every map reads UV set `texture.channel` (default 0 = `uv`).
// For a separate AO unwrap, add a `uv1` attribute and set aoTexture.channel = 1.
```
