...
> **Vendored reference** (see `../VENDORED-threejs.md`). Repo rules win: three.js
> only in `src/main.ts` and `src/view/**`; graybox primitives; no postprocessing or
> shader polish unless the prototype's question is about visuals. Game state
> advances in fixed ticks in `src/sim.ts`, never from a frame's delta, and never
> uses `Math.random()`: the `Timer`/delta and `Math.random` snippets below are for
> view-only effects.
...
// No constructor arguments in current three; the composer sizes it.
...
// FilmPass(intensity, grayscale); the scanline arguments were removed
...
```javascript
// Prefer OutputPass over GammaCorrectionShader: it applies the renderer's
// tone mapping and output color space, like a plain renderer.render().
...
// HalftonePass(params); the width/height arguments were removed
...
customPass.uniforms.time.value = timer.getElapsed();
```
...
## WebGPU Post-Processing (node-based, WebGPURenderer only)
...
// RenderPipeline was called PostProcessing before r183.
const renderPipeline = new THREE.RenderPipeline(renderer);
renderPipeline.outputNode = scenePass.add(bloomNode);
