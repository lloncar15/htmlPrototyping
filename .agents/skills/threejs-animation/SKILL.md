...
// THREE.Clock is deprecated since r183; Timer replaces it.
const timer = new THREE.Timer();

function animate(timestamp) {
  timer.update(timestamp); // Once per frame, before reading
...
// String track (discrete string properties; morph influences use
// NumberKeyframeTrack, see Morph Targets below)
new THREE.StringKeyframeTrack(".name", [0, 1], ["idle", "walk"]);
```
...
- `threejs-fundamentals` - Timer and animation loop
