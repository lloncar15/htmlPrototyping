...
three.js has no `ContactShadows` class (that is a `@react-three/drei`
component). For a cheap graybox stand-in, put a dark, transparent
`CircleGeometry` blob under each object:
...
blob.position.y = 0.01; // Just above the ground, avoids z-fighting
scene.add(blob);
```
...
lightProbe.copy(
  await LightProbeGenerator.fromCubeRenderTarget(renderer, cubeCamera.renderTarget), // async
);
```
