...
new HDRLoader().load("environment.hdr", (texture) => {
...
```javascript
// Since r151, maps read the UV set named by texture.channel
// (0 = "uv", 1 = "uv1", ...). aoMap works on "uv" by default.
// Only for a separate AO unwrap:
...
const material = new THREE.MeshStandardMaterial({
  ...
  // Ambient occlusion (Linear, uses `uv` unless aoMap.channel is set)
  aoMap: aoTexture,
  aoMapIntensity: 1,
